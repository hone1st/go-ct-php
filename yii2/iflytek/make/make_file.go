...
func (c *Controller) Make() {
	...
	filepath := other.GetFilePath(c.Root, c.NameSpace, c.Name+"Controller.php")
...
func (s *Service) Make() {
	...
	serviceFilepath := other.GetFilePath(s.Root, s.NameSpace, s.Name+"Service.php")
	serviceImplFilepath := other.GetFilePath(s.Root, s.NameSpace, "impl", s.Name+"ServiceImpl.php")
	if WriteFile(serviceFilepath, serviceContent) && WriteFile(serviceImplFilepath, serviceImplContent) {
		diPath := other.GetFilePath(s.Root, "common", "config", "di-container.php")
...
func (f *Form) Make() {
	...
	filepath := other.GetFilePath(f.Root, f.NameSpace, f.Name+"Form.php")
	WriteFile(filepath, content)
}
...
func (m *Module) Make() {
	...
	filepath := other.GetFilePath(m.Root, m.NameSpace, m.Name, "Module.php")
	content := fmt.Sprintf(template.Module, m.NameSpace+"\\"+m.Name, "\\"+parent)
	if WriteFile(filepath, content) {
		parentC, _ := ioutil.ReadFile(other.GetFilePath(m.Root, parent+".php"))
		...
		if !strings.Contains(string(parentC), dest) {
			...
			newC :=
				strings.Replace(
					...
					1)
			_ = ioutil.WriteFile(other.GetFilePath(m.Root, parent+".php"), []byte(newC), 0777)
		}
	}
}
