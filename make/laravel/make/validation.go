...
func (v *Validation) Make() {
	// 固定路径
	// 检测文件是否存在
	controllerFilePath := util.GetFilePath(v.Root, v.Namespace, v.Name+"Controller.php")
	controllerData, err := ioutil.ReadFile(controllerFilePath)
	if err != nil {
		log.Fatalf("控制器文件不存在：【%s】 无法生成验证层文件", controllerFilePath)
	}
	controllerPath := util.GetFilePath(v.Root, v.Namespace)
	split := strings.Split(controllerPath, "App/Http/Controllers")

	if len(split) == 2 {
		validationPath := util.GetFilePath(v.Root, "app", "Http", "Validations", split[1], v.Name+"Validation.php")
		// 检测是否存在
