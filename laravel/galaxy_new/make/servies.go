...
func (m *Services) Make() {
	...
	content := strings.ReplaceAll(template.Services, "{$namespace}", m.Namespace)
	content = strings.ReplaceAll(content, "{$namespace_parent}", strings.Replace(m.Namespace, "\\Services", "", 1))
